				   ...
				   bool p2p_disable_6ghz)
{
	...
	for (op = 0; global_op_class[op].op_class; op++) {
		...
		bool is_dfs_allowed = ((is_p2p_dfs_chan_enabled(wpa_s->global->p2p) || wpa_s->allow_p2p_assisted_dfs)
						&& is_dfs_global_op_class(o->op_class));

		wpa_printf(MSG_DEBUG, "p2p_dfs: opclass %d is_dfs_allowed %d", o->op_class, is_dfs_allowed);
